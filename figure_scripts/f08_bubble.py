...
def draw_bubble(year: int):
    ...
    ax.format(
        xlabel="Population Using Generative AI (%)",
    ...
    out = OUTDIR / f"f08_bubble_{year}.svg"
    ...
    out_leg = OUTDIR / f"f08_bubble_{year}_legend.svg"
...
def draw_bubble_labeled(year: int):
    ...
    out = OUTDIR / f"f08_bubble_labeled_{year}.svg"
    fig.savefig(out, dpi=300, bbox_inches="tight")
    pplt.close(fig)
    print(f"  -> {out}")
