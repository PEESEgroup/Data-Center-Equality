...
def load_iso_price_ratios_baseline(dir_path: Path, years: list[int]) -> dict:
    ...
    def _iso_from_filename(p):
        return re.sub(r"[^A-Za-z]", "", p.stem.split("_")[0]).upper()

    def _coerce_year_cols(df):
        mapper = {}
        for c in df.columns:
            cs = str(c).strip()
    ...
    for xlsx in sorted(dir_path.glob("*_zone_prices.xlsx")):
        ...
        for sheet in sheets:
            ...
            zone_clean = _clean_zone_local(sheet, iso)
            if not zone_clean:
                continue

            for y in years:
                v = s_total.get(y, s_total.get(str(y), np.nan))
                if pd.notna(v) and np.isfinite(float(v)):
                    ratios[iso].setdefault(zone_clean, {})[y] = float(v) / base_total
